...
def get_d4rl_dataset_with_mc_calculation(env, reward_scale, reward_bias, clip_action, gamma):
    try:
        ...
        raise ImportError(
            "D4RL is required for AntMaze datasets. On Windows this project typically "
            "needs WSL or Linux because D4RL depends on MuJoCo tooling that is not "
            "reliably available natively."
        ) from exc
...
def get_custom_npz_dataset_with_mc_calculation(dataset_path, gamma, reward_scale=1.0, reward_bias=0.0, clip_action=None):
    ...
    if missing:
        raise ValueError(f"Missing keys in dataset {dataset_path}: {missing}")

    observations = dataset['observations'].astype(np.float32)
    actions = dataset['actions'].astype(np.float32)
    next_observations = dataset['next_observations'].astype(np.float32)
    rewards = dataset['rewards'].reshape(-1).astype(np.float32)
    dones = dataset['dones'].reshape(-1).astype(np.float32)
