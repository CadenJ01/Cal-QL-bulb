...
class ConservativeSAC(object):
    ...
    def load_snapshot(self, snapshot):
        for key in ["policy", "qf1", "qf2"]:
            self._train_states[key] = self._train_states[key].replace(params=snapshot["train_params"][key])
        if "log_alpha" in self._train_states and "log_alpha" in snapshot["train_params"]:
            self._train_states["log_alpha"] = self._train_states["log_alpha"].replace(
                params=snapshot["train_params"]["log_alpha"]
            )
        if "log_alpha_prime" in self._train_states and "log_alpha_prime" in snapshot["train_params"]:
            self._train_states["log_alpha_prime"] = self._train_states["log_alpha_prime"].replace(
                params=snapshot["train_params"]["log_alpha_prime"]
            )
        self._target_qf_params = snapshot["target_qf_params"]
        self._total_steps = snapshot.get("total_steps", self._total_steps)
